...
class VRobotNode(VRobotNodeBase):
    ...
    def update(self):
        ...
        if isNewState:
            self.state = state
            # print("ts state =", self.state.timestamp)

        isNewImgLeft, imgStateLeft = self.read_new_image("left", self.imgStateLeft)
        if isNewImgLeft:
        ...
        isNewImgRight, imgStateRight = self.read_new_image("right", self.imgStateRight)
        if isNewImgRight:
        ...
        isNewImgDown, imgStateDown = self.read_new_image("down", self.imgStateDown)
        if isNewImgDown:
